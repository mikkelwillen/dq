...
using Simulator = SimulatorBasic<For, fptype>;
}
...
int main(int argc, char* argv[]) {
  ...
  Circuit<GateQSim<fptype>> circuit;
  ...
  using Fuser = MultiQubitGateFuser<IO, GateQSim<fptype>>;
  ...
  int runs = 0;
  int runs_min = 10;
  long long int microseconds_min = 1000000;
  long long int total_microseconds = 0;
  while (total_microseconds < microseconds_min || runs < runs_min) {
    state_space.SetStateZero(state);
    auto start = steady_clock::now();
    if (!Runner::Run(param, Factory(opt.num_threads), circuit, state)) {
      std::cerr << "Run " << runs << " failed." << std::endl;
      return 1;
    }
    auto finish = steady_clock::now();
    total_microseconds += duration_cast<microseconds>(finish - start).count();
    runs++;
  }
